...
export default function Programmers() {
  ...
  const [featProgId, setProgId] = useState();
  ...
  const getNameOfFeatured = () => {
    ...
    return programmersList.find(elem => elem.id === featProgId).name;
  };

  const style = {
    fontSize: '1.5em',
    marginTop: '0.5em',
    color: featProgId ? 'gold' : 'royalblue', // 🤔 color turns to gold, when celebrating
  };
      ...
      <div className='programmers'>
        {
          ...
          programmersList.map(dev =>
            <div className='programmer' key={dev.id}>
              {dev.name} <button onClick={() => { setProgId(dev.id === featProgId ? null : dev.id)}}>Feature</button>
            </div>
          )
        }
      </div>
